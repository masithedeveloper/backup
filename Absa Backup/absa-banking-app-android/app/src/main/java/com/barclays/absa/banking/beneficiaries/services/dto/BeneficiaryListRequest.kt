...
class BeneficiaryListRequest<T>(beneficiaryType: String, responseListener: ExtendedResponseListener<T>)
    : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder()
                .put(OP0310_BENEFICIARY_LIST)
                .put(BeneficiariesService.PAYMENT_TYPE, beneficiaryType)
                .build()
