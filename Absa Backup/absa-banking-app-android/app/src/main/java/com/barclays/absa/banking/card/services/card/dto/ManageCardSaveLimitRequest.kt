...
class ManageCardSaveLimitRequest<T>(manageCardConfirmLimit: ManageCardConfirmLimit, responseListener: ExtendedResponseListener<T>)
    : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder()
                .put(OP0123_CARD_CHANGE_LIMITS_RESULT)
                .put(Transaction.CARD_CONFIRM_TXN_REF_ID, manageCardConfirmLimit.txnRefID)
                .build()
