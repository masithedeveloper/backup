/*
 * Copyright (c) 2018 Absa Bank Limited, All Rights Reserved.
 *
 * This code is confidential to Absa Bank Limited and shall not be disclosed
 * outside the Bank without the prior written permission of the Absa Legal
 *
 * In the event that such disclosure is permitted the code shall not be copied
 * or distributed other than on a need-to-know basis and any recipients may be
 * required to sign a confidentiality undertaking in favor of Absa Bank
 * Limited
 *
 */
package com.barclays.absa.banking.presentation.genericResult;

import android.os.Bundle;

import com.barclays.absa.banking.card.ui.creditCardReplacement.ReplacementAssistanceDialogFragment;

public class DialogGenericResultActivity extends GenericResultActivity {

    @Override
    public void onCreate(Bundle savedInstanceState) {
        boolean shouldClearAccountCache = getIntent().getBooleanExtra(IS_SUCCESS, false);

        topOnClickListener = view -> {
            ReplacementAssistanceDialogFragment replacementAssistanceDialogFragment = new ReplacementAssistanceDialogFragment();
            replacementAssistanceDialogFragment.show(getFragmentManager(), "");
        };

        bottomOnClickListener = v -> {
            if (shouldClearAccountCache) {
                loadAccountsClearingAccountProfileAndShowHomeScreen();
            } else {
                loadAccountsAndGoHome();
            }
            showProgressDialog();
        };
        super.onCreate(savedInstanceState);
    }
}
