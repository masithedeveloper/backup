...
class DebitOrderApprovalExtendedResponseListener(private val viewModel: DebiCheckViewModel) : ExtendedResponseListener<DebitOrderAuthoriseResponse>() {
    override fun onSuccess(successResponse: DebitOrderAuthoriseResponse) {
        if (BMBConstants.SUCCESS.equals(successResponse.transactionStatus, true)) {
            viewModel.debitOrderAcceptResponse.value = successResponse
        } else {
