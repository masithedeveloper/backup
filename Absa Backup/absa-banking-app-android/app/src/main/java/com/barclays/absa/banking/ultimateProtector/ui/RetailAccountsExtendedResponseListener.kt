...
class RetailAccountsExtendedResponseListener(val retailAccountsLiveData: MutableLiveData<List<RetailAccount>>) : ExtendedResponseListener<RetailAccountsResponse>() {
    override fun onSuccess(successResponse: RetailAccountsResponse) {
        retailAccountsLiveData.value = successResponse.retailAccountsList
    }
}