...
class MonthlyIncomeExtendedResponseListener(private var viewModel: ManageProfileViewModel) : ExtendedResponseListener<LookupResult>() {
    override fun onSuccess(successResponse: LookupResult) {
        if (FAILURE.equals(successResponse.transactionStatus, ignoreCase = true)) {
            ...
            viewModel.monthlyIncomeLookupResult.value = successResponse
        }
    }
