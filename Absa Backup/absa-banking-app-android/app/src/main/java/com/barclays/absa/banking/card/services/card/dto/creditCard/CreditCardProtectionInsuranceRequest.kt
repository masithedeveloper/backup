...
import com.barclays.absa.banking.card.services.card.dto.CreditCardService.OP0864_FETCH_CREDIT_CARD_PROTECTION_INSURANCE_QUOTE
import com.barclays.absa.banking.framework.ExtendedRequest
import com.barclays.absa.banking.framework.ExtendedResponseListener
import com.barclays.absa.banking.framework.api.request.params.RequestParams
import com.barclays.absa.banking.framework.api.request.params.TransactionParams.Transaction

class CreditCardProtectionInsuranceRequest<T>(creditCardNumber: String, responseListener: ExtendedResponseListener<T>)
    : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder()
                .put(OP0864_FETCH_CREDIT_CARD_PROTECTION_INSURANCE_QUOTE)
                .put(Transaction.ACCOUNT_NO, creditCardNumber)
                .build()

        mockResponseFile = CardMockFactory.creditCardProtection()
        printRequest()
    }
