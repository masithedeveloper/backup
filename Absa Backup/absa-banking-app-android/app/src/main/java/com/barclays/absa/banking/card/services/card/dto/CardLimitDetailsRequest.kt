...
class CardLimitDetailsRequest<T>(card: Card, responseListener: ExtendedResponseListener<T>)
    : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder()
                .put(OP0121_CARD_FETCH_CHANGE_LIMIT)
                .put(Transaction.CARD_CHANGE_LIMIT_CARD_NUMBER, card.cardNumber)
                .put(Transaction.CARD_CHANGE_LIMIT_CARD_TYPE, card.cardType)
                .build()
