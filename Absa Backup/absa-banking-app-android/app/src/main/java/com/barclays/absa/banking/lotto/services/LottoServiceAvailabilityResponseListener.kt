...
class LottoServiceAvailabilityResponseListener(val viewModel: LottoViewModel) : ExtendedResponseListener<LottoServiceAvailabilityResponse>() {

    override fun onSuccess(successResponse: LottoServiceAvailabilityResponse) {
        performDefaultResponseHandling(successResponse) {
            viewModel.serviceAvailabilityLiveData.value = successResponse
        }
    }
