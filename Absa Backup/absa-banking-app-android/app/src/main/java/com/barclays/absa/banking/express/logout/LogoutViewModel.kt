 ...
 *
 */
package com.barclays.absa.banking.express.logout
...
class LogoutViewModel : ExpressBaseViewModel() {

    override val repository by lazy { LogoutRepository() }

    fun logout() {
        viewModelScope.launch {
            if (BMBApplication.getInstance().userLoggedInStatus) {
                failureLiveData.observeForever { }
                repository.submitRequest<BaseResponse>()
            }
        }
