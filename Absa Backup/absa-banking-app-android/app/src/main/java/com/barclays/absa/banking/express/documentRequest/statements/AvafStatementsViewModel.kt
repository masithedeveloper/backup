...
class AvafStatementsViewModel : ExpressBaseViewModel() {
    override val repository by lazy { AvafStatementsRepository() }
    ...
    fun requestDocument(requestDetails: DocumentRequestDetails, account: String) {
        documentRequestRequestLiveData = liveData(Dispatchers.IO) { repository.submitDocumentRequest(requestDetails, account)?.let { emit(it) } }
    }
}