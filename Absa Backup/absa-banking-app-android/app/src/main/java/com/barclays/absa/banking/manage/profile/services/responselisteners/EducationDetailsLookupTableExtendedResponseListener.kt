...
class EducationDetailsLookupTableExtendedResponseListener(private var mangeProfileViewModel: ManageProfileViewModel) : ExtendedResponseListener<LookupResult>() {
    override fun onSuccess(successResponse: LookupResult) {
        if (FAILURE.equals(successResponse.transactionStatus, ignoreCase = true)) {
            mangeProfileViewModel.lookupFailure.value = successResponse.transactionMessage
        } else {
            mangeProfileViewModel.populateEducationAndEmploymentDetailsLookupValues(successResponse)
        }
    }

    override fun onFailure(failureResponse: ResponseObject?) {
        mangeProfileViewModel.lookupFailure.value = failureResponse?.errorMessage
    }
}