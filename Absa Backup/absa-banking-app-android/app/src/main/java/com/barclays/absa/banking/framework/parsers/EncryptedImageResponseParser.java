...
import com.barclays.absa.banking.boundary.model.EncryptedImageResponse;
...
public class EncryptedImageResponseParser {

    private static final String TAG = EncryptedImageResponseParser.class.getSimpleName();

    public ResponseHeader getResponseHeader(String encryptedImageResponseString) {

        final ResponseHeader responseHeader = new ResponseHeader();

        try {
            // Get encrypted services and nonce
            EncryptedImageResponse encryptedImageResponse = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).readValue(encryptedImageResponseString, EncryptedImageResponse.class);

            // Decrypt
            String decryptedResponseString = AESEncryption.decrypt(
                    BMBApplication.getInstance().getIVal(),
                    BMBApplication.getInstance().getStaticKey(),
                    encryptedImageResponse.getData()
            );
