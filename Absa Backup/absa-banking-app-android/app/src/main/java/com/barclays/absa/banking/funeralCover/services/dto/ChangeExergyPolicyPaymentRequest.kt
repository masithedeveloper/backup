...
class ChangeExergyPolicyPaymentRequest<T>(changePaymentDetails: ChangePaymentDetails, responseListener: ExtendedResponseListener<T>) : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder(InsurancePolicyService.OP2184_CHANGE_EXERGY_PAYMENT_DETAILS_OPCODE)
                ...
                .put(ChangePolicyPaymentRequestParameters.BANK_ID, changePaymentDetails.bankId)
                .put(ChangePolicyPaymentRequestParameters.BRANCH_ID, changePaymentDetails.branchId)
                .build()
