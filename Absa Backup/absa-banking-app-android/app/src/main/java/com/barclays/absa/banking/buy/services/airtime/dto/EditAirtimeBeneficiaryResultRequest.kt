...
import com.barclays.absa.banking.buy.services.airtime.PrepaidService.OP0333_EDIT_AIRTIME_RESULT
import com.barclays.absa.banking.framework.ExtendedRequest
import com.barclays.absa.banking.framework.ExtendedResponseListener
import com.barclays.absa.banking.framework.api.request.params.RequestParams
import com.barclays.absa.banking.framework.api.request.params.TransactionParams.Transaction

class EditAirtimeBeneficiaryResultRequest<T>(referenceNumber: String, hasImage: String,
                                             responseListener: ExtendedResponseListener<T>)
    : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder()
                .put(OP0333_EDIT_AIRTIME_RESULT)
                .put(Transaction.SERVICE_TXNREF, referenceNumber)
                .put(Transaction.HAS_IMAGE, hasImage)
                .build()

        mockResponseFile = BeneficiariesMockFactory.editAirtimeResults()
        printRequest()
    }
