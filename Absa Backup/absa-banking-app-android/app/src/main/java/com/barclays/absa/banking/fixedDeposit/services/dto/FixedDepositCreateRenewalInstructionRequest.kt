...
class FixedDepositCreateRenewalInstructionRequest<T>(renewalInstructionData: FixedDepositRenewalInstructionData, responseListener: ExtendedResponseListener<T>) : ExtendedRequest<T>(responseListener) {

    init {
        params = RequestParams.Builder(FixedDepositService.OP2163_CREATE_FIXED_DEPOSIT_RENEWAL_INSTRUCTION)
        ...
        mockResponseFile = "fixed_deposit/op2163_fixed_deposit_create_renewal_instruction.json"
        printRequest()
    }

    @Suppress("UNCHECKED_CAST")
    override fun getResponseClass(): Class<T> = FixedDepositCreateRenewalInstructionResponse::class.java as Class<T>
