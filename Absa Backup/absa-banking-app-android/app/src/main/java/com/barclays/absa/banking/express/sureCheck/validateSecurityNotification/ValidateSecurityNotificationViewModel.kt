...
class ValidateSecurityNotificationViewModel : ExpressBaseViewModel() {
    override val repository by lazy { ValidateSecurityNotificationRepository() }

    lateinit var validateSecurityNotificationLiveData: LiveData<SecurityNotificationResponse>

    fun validateSecurityNotification(securityNotificationRequest: SecurityNotificationRequest) {
        validateSecurityNotificationLiveData = liveData(Dispatchers.IO) { repository.validateSecurityNotification(securityNotificationRequest)?.let { emit(it) } }
    }
}